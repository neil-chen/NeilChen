...
class Common_util_pub
{
	...
	{
		Logger::debug('支付返回结果11121345465'.$xml);	
        //将XML转为array        
...
class JsApi_pub extends Common_util_pub
{
	...
	/*
		*设置拉起收货地址jsapi的参数
	*/
	public function getAddressParameters($accessToken="")
	{
		/*
		$jsApiObj["appId"] = 'wx17ef1eaef46752cb';
		$timeStamp = '1384841012';
	    $jsApiObj["timeStamp"] = "$timeStamp";
	    $jsApiObj["nonceStr"] = "123456";
		$jsApiObj["url"] = "http://open.weixin.qq.com/";
		$jsApiObj["accessToken"] = "OezXcEiiBSKSxW0eoylIeBFk1b8VbNtfWALJ5g6aMgZHaqZwK4euEskSn78Qd5pLsfQtuMdgmhajVM5QDm24W8X3tJ18kz5mhmkUcI3RoLm7qGgh1cEnCHejWQo8s5L3VvsFAdawhFxUuLmgh5FRA";
	    $jsApiObj["addrSign"] = $this->getAddressSign($jsApiObj);
		...
		*/
	    ...
	    $jsApiObj["timeStamp"] = "$timeStamp";
	    $jsApiObj["nonceStr"] = $this->createNoncestr();
		$jsApiObj["url"] = "http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
		
		$jsApiObj["accessToken"] = $accessToken;
		Logger::debug('获取微信收货地址参数'.json_encode($jsApiObj));	
	    $jsApiObj["addrSign"] = $this->getAddressSign($jsApiObj);
		$jsApiObj["signType"] = "sha1";
		$jsApiObj["scope"] = "jsapi_address";
		unset($jsApiObj["url"]);
	 ...
	 * 	作用：生成获取微信收货地址签名
	 */
	public function getAddressSign($Obj)
	{
		foreach ($Obj as $k => $v)
		{
			$Parameters[$k] = $v;
		...
		ksort($Parameters);
		$String = $this->formatBizQueryParaMapAddr($Parameters, false);
		$String = sha1($String);
		return $String;
	}
	public function formatBizQueryParaMapAddr($paraMap, $urlencode)
	{
		$buff = "";
		ksort($paraMap);
		foreach ($paraMap as $k => $v)
